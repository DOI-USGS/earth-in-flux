...
When preparing to migrate a repo built from this template to DGEC, the name of the GitHub repo (`{project_name}`) in the DGEC required files `'code.json'` and `'CONTRIBUTING.md'` will need to be updated, so that the value of `VITE_APP_TITLE` is used to replace `{project_name}`. The `VITE_APP_GITHUB_REPOSITORY_LINK` environment variable specified in `.env` must reflect the name of the GitHub repo, e.g., `'https://github.com/DOI-USGS/vue3-template'`.
...
1. Update project-specific environment variables, titles, and paths
    ...
    * [x] Update `"name"` in `'package.json'`.
    * [x] Update project name parameter (line 11) in `'jenkins/Jenkinsfile.build'`
    ...
    * [x] Update the `{project-name}` variable throughout the DGEC required files `'code.json'` and `'CONTRIBUTING.md'`, using the value of `VITE_APP_TITLE` to replace `{project_name}`
