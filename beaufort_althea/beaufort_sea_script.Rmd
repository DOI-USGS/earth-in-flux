...
pca_data <- raw_pct_abundance[,65:123] 
names(pca_data) <- substr(names(pca_data), 1, 20)
rownames(pca_data) <- round(raw_pct_abundance$`Calendar AGE`, 0)

res.pca <- prcomp(pca_data, retx = TRUE)
...
# Both variables and individuals
...
# forams
fpca_data <- forams_raw[c(1:25, 27:120),c(24:42)] 
names(fpca_data) <- substr(names(fpca_data), 1, 20)
rownames(fpca_data) <- round(forams_raw$`calendar yr`[c(1:25, 27:120)], 1)

fres.pca <- prcomp(fpca_data, retx = TRUE)

# Graph of individuals
fviz_pca_ind(fres.pca,
             col.ind = "cos2", # Color by the quality of representation
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE     # Avoid text overlapping
             )
...
# Both variables and individuals
fviz_pca_biplot(fres.pca, repel = TRUE,
                col.var = "#2E9FDF", # Variables color
                col.ind = "#696969"  # Individuals color
                )
```
